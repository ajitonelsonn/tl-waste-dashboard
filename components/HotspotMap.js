...
const HotspotMap = ({ hotspots = [], selectedHotspot, onHotspotSelect }) => {
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current && mapRef.current) {
      mapInstanceRef.current = L.map(mapRef.current).setView(DEFAULT_CENTER, 11);
  ...
  useEffect(() => {
    ...
    Object.keys(circlesRef.current).forEach(id => {
      const hotspotExists = hotspots.some(h => h.hotspot_id.toString() === id);
      if (!hotspotExists) {
    ...
    if (selectedHotspot && isValidCoordinate(selectedHotspot.center_latitude, selectedHotspot.center_longitude)) {
      ...
      const totalLat = validHotspots.reduce((sum, h) => sum + parseNumeric(h.center_latitude), 0);
      const totalLng = validHotspots.reduce((sum, h) => sum + parseNumeric(h.center_longitude), 0);
      center = [totalLat / validHotspots.length, totalLng / validHotspots.length];
    }
    ...
    validHotspots.forEach(hotspot => {
      ...
      if (circlesRef.current[hotspotId]) {
        ...
        // Create new circle
        const circle = L.circle(position, circleOptions).addTo(mapInstanceRef.current);
        ...
        circle.bindPopup(`
          <div class="max-w-xs">
            ...
            <div class="text-sm">
              ...
              ${hotspot.average_severity ? `<div>Avg. Severity: ${formatNumber(hotspot.average_severity)}/10</div>` : ''}
