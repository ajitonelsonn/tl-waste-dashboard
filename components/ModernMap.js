...
const ModernMap = ({ data, simplified = false, selectedHotspot = null, onHotspotSelect }) => {
    ...
    if (!mapInstanceRef.current && mapRef.current) {
      ...
      if (!simplified) {
        ...
        legendControl.onAdd = function() {
          const div = L.DomUtil.create('div', 'legend bg-white p-3 rounded-lg shadow-md text-sm');
          div.innerHTML = `
  ...
  useEffect(() => {
    ...
    validReports.forEach(report => {
      ...
      const marker = L.marker(
        position,
        { icon: createMarkerIcon(report.status, parseNumeric(report.severity_score)) }
      ).addTo(mapInstanceRef.current);
      
      // Add modern styled popup with image if available
      const popupContent = `
        <div class="max-w-xs">
          <div class="font-semibold text-emerald-700 mb-1">Report #${report.report_id}</div>
          <div class="text-xs text-gray-500 mb-2">${new Date(report.report_date).toLocaleDateString()}</div>
          ...
          <div class="text-sm">
            ...
            ${report.severity_score ?
              `<div class="mb-1">
                ...
                <span class="${
                  report.severity_score > 7 ? 'text-red-600' :
                  report.severity_score > 4 ? 'text-amber-600' :
                  'text-green-600'
    ...
    validHotspots.forEach(hotspot => {
      ...
      const isSelected = selectedHotspot && hotspot.hotspot_id === selectedHotspot.hotspot_id;
      ...
      const hotspotPopupContent = `
        <div class="max-w-xs">
          ...
          <div class="text-xs text-gray-500 mb-2">First reported: ${hotspot.first_reported}</div>
          ...
          <div class="grid grid-cols-2 gap-2 text-sm">
            ...
            ${hotspot.average_severity ? 
              `<div>
                <span class="font-medium">Severity:</span> 
                <span class="${
                  parseNumeric(hotspot.average_severity) > 7 ? 'text-red-600' :
                  parseNumeric(hotspot.average_severity) > 4 ? 'text-amber-600' :
                  'text-green-600'
            ...
            <div>
              <span class="font-medium">Radius:</span> ${formatNumber(hotspot.radius_meters / 1000)}km
            </div>
          </div>
          
          <div class="mt-3 text-center">
            <a href="/hotspots?id=${hotspot.hotspot_id}" class="inline-block px-3 py-1.5 text-xs bg-red-600 text-white rounded hover:bg-red-700 transition-colors">
              View Details
            </a>
          </div>
        </div>
    ...
    if (selectedHotspot && isValidCoordinate(selectedHotspot.center_latitude, selectedHotspot.center_longitude)) {
      const position = [
    ...
    // Otherwise adjust map view to fit all points if we have valid points
      ...
      const allPoints = [
        ...validReports.map(r => [parseNumeric(r.latitude), parseNumeric(r.longitude)]),
        ...validHotspots.map(h => [parseNumeric(h.center_latitude), parseNumeric(h.center_longitude)])
      ];
      ...
      {simplified && data && (
        <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-2 pointer-events-auto">
          <div className="bg-white py-1.5 px-3 rounded-lg shadow-md flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{data.reports ? data.reports.length : 0} Reports</span>
          </div>
          <div className="bg-white py-1.5 px-3 rounded-lg shadow-md flex items-center gap-2">
            <Layers className="w-4 h-4 text-red-500" />
            <span className="text-sm font-medium">{data.hotspots ? data.hotspots.length : 0} Hotspots</span>
          </div>
        </div>
      )}
    </div>
