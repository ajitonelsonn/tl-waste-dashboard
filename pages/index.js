...
import { useStatsOverview, useMapData, useReports, useTrends } from '../lib/api';
import dynamic from 'next/dynamic';
import { ArrowUpRight, Calendar, MapPin, AlertTriangle, RefreshCw, Filter, Layers, BarChart2 } from 'lucide-react';
...
export default function ModernDashboard() {
  ...
  const { statsData, isLoading: statsLoading, refresh: refreshStats } = useStatsOverview();
  const { mapData, isLoading: mapLoading, refresh: refreshMap } = useMapData();
  const { reportsData, isLoading: reportsLoading, refresh: refreshReports } = useReports(1, 5);
  const { trendData, isLoading: trendLoading, refresh: refreshTrends } = useTrends(selectedTrend, 30);

  // Combined loading state
  const isLoading = statsLoading || mapLoading || reportsLoading || trendLoading;
  ...
  const summaryStats = statsData ? {
    ...
    severity: {
      count: statsData.avg_severity ? statsData.avg_severity.toFixed(1) + '/10' : 'N/A',
      label: 'Avg. Severity',
  ...
  const trendingData = trendData?.report_trends || (statsData?.daily_reports || []);

  return (
    <Layout>
      <Head>
        <title>TL Waste Monitoring | Modern Dashboard</title>
        <meta name="description" content="Digital Waste Monitoring Network for Timor-Leste" />
      </Head>
      ...
      <div className="px-6 py-8 max-w-7xl mx-auto">
        ...
        {refreshing && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 mb-6 flex items-center shadow-sm">
            ...
            <span className="text-sm text-emerald-700">Refreshing dashboard data...</span>
          </div>
        )}

        {/* Stats Grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {summaryStats && Object.entries(summaryStats).map(([key, stat]) => (
            <div 
              key={key} 
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-semibold text-gray-800">Waste Distribution Map</h2>
              </div>
              <Link 
                href="/map" 
                  ...
                  {recentReports.map((report) => (
                    <div key={report.report_id} className="p-4 hover:bg-gray-50">
                      <div className="flex justify-between">
                        <Link href={`/reports/${report.report_id}`} className="text-emerald-600 hover:text-emerald-800 font-medium">
                          #{report.report_id}
                        </Link>
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                          ${report.status === 'resolved' ? 'bg-green-100 text-green-800' :
                            report.status === 'analyzed' ? 'bg-blue-100 text-blue-800' :
                            ...
                            <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                              <div 
                                className={`h-1.5 rounded-full ${
                                  report.severity_score > 7 ? 'bg-red-500' :
                                  report.severity_score > 4 ? 'bg-yellow-500' :
                                  'bg-green-500'
                                }`}
                                style={{ width: `${report.severity_score * 10}%` }}
                              ></div>
                            </div>
                            <span className="text-xs font-medium">{report.severity_score}/10</span>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-1">
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-lg ${selectedTrend === 'daily' 
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-lg ${selectedTrend === 'weekly' 
                      ? 'bg-emerald-100 text-emerald-800 font-medium' 
                      : 'text-gray-600 hover:bg-gray-100'}`}
                  >
                    Weekly
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-lg ${selectedTrend === 'monthly' 
                      ? 'bg-emerald-100 text-emerald-800 font-medium' 
                      : 'text-gray-600 hover:bg-gray-100'}`}
                  >
                    Monthly
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold text-gray-800">Active Waste Hotspots</h2>
            </div>
            <Link 
              href="/hotspots" 
              className="text-sm text-emerald-600 hover:text-emerald-800 flex items-center gap-1"
                      ...
                      <div className="mt-2 flex justify-between">
                        <div className="text-sm text-red-700">
                          {hotspot.average_severity && (
                            <span>Severity: {parseFloat(hotspot.average_severity).toFixed(1)}/10</span>
                          )}
                        </div>
                        <div className="text-sm text-red-700">
                          Radius: {(parseFloat(hotspot.radius_meters) / 1000).toFixed(1)}km
                        </div>
                      </div>
                    </div>
                  </Link>
        ...
        <div className="mt-8 bg-emerald-50 rounded-xl p-6 shadow-sm border border-emerald-100">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h3 className="text-xl font-bold text-emerald-800 mb-2">Help Keep Timor-Leste Clean</h3>
              <p className="text-emerald-700">Report waste incidents and help track environmental impact in your community</p>
            </div>
